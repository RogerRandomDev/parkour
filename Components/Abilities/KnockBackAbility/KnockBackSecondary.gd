...
func trigger():
	loadAbilityVisuals()
	await root.get_tree().create_timer(0.125).timeout
...
func loadAbilityVisuals()->void:
	...
	#tween.tween_property(mesh,'scale',Vector3(0.0,0.0,0.0),0.5)
	for i in range(0,6):
		var scale_of=1-(float(i+1)/6)
		#tween.tween_property(mesh,'scale',Vector3(scale_of,scale_of,scale_of),0.0)
		tween.tween_callback(func():
			mesh.scale=Vector3(scale_of,scale_of,scale_of)
			meshEffect.set_shader_parameter('rippleProgress',1-(float(i+1)/6))
			)
